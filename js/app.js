...
const updateTaxAndCharge = () => {
   ...
   if (priceConverted > 500) {
      setInnerValue('delivery-charge', 60);
      setInnerValue('total-tax', priceConverted * 0.4);
   }
  else if (priceConverted > 400) {
      setInnerValue('delivery-charge', 50);
      setInnerValue('total-tax', priceConverted * 0.3);
   }
  else if (priceConverted > 200) {
      setInnerValue('delivery-charge', 30);
      setInnerValue('total-tax', priceConverted * 0.2);
   }
};
...
document.getElementById("search-btn").addEventListener("click", function () {
   ...
   const searchedProduct = arr[0].filter((p) =>
     p.title.includes(`${inputField}`)//getegoryr name gula jodi input e likha letter gula dhara start hoy tahole segula dekhaw
   );
